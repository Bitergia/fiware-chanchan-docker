...
function _authzforce_sync () {
    ...
    sed -i '/from deployment import keystone/a from deployment import access_control_sync' ${KEYROCK_HOME}/fabfile.py
...
function _data_provision () {
    if [ -e /initialize-provision ] ; then
        ...
        if check_file ${PROVISION_FILE} 10 ; then
            ...
            cp ${PROVISION_FILE} ${KEYROCK_HOME}/deployment/keystone_provision.py
        else
            echo "Launching default provision file"
        fi
        sed -i "/from deployment import keystone/a from deployment import ${FILE}" ${KEYROCK_HOME}/fabfile.py
